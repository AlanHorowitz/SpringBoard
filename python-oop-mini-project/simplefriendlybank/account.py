...
class SavingsAccount(Account):
    ...
    def __init__(self, customer_id=None):
        ...
        self._id = SavingsAccount.SAVINGS_ID_PREFIX + str(
            SavingsAccount.next_savings_account_id
        )
...
class CheckingAccount(Account):
    ...
    checking_open_metadata = {
        "open": (
            ...
            [
                ...
                (
                    "number_starter_checks",
                    "Please enter your initial number of checks (default is 10): ",
                    "non_negative_int",
                ),
            ],
        )
    }
    checking_order_checks_metadata = {
        "order_checks": (
            "Order replacement checks ($5 per 50 checks will be deducted from balance) ",
            [
                (
                    "number_of_checks",
                    "Please enter number of new checks: ",
                    "non_negative_int",
                )
            ],
        )
    }
    checking_write_check_metadata = {
        "write_check": (
            "Write a check",
    ...
    def __init__(self, customer_id=None):
        super().__init__(customer_id)
        self._id = CheckingAccount.CHECKING_ID_PREFIX + str(
            CheckingAccount.next_checking_account_id
        )
    ...
    def order_checks(self, number_of_checks=0):
        cost_of_checks = number_of_checks * 0.1
        if self._balance >= cost_of_checks:
            self._balance -= cost_of_checks
            self._remaining_checks += number_of_checks
        else:
            raise TransactionFailedException(
                f'Insufficient balance: {"{:.2f}".format(self._balance)} to purchase checks'
            )

    def write_check(self, check_amount=0.00):
        if check_amount > self._balance:
            raise TransactionFailedException(
                f'Insufficient balance: {"{:.2f}".format(self._balance)} to write check for: {"{:.2f}".format(check_amount)}.'
            )
        elif self._remaining_checks < 1:
            raise TransactionFailedException(
                "No checks remaining. Please purchase additional checks."
            )
        else:
