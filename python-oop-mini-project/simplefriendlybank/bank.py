    ...
    while True:
        ...
        if parm_type == "non_negative_float":
            try:
                ...
                if typed_value < 0:
                    print(
                        "Value must be a non-negative float (e.g 100.00).  Please try again"
                    )
                    continue
            except ValueError:
                print(
...
class Bank:
    ...
    def menu_get_main_activity(self):
        ...
        options = (
            ("Open a new account", Bank.OPEN_NEW_ACCOUNT),
    ...
    def get_new_customer(self):
        ...
        if (
            customer in self.customers.values()
        ):  # customer equality based on first_name, last_name ssn
    ...
    def get_validated_customer(self):
        ...
        if option == Bank.YES:
            ...
            raise self.BankSystemError(
                "get_validated_customer: Received answer other than Yes/No"
            )
    ...
    def run_transaction(self):
        ...
        while another_transaction == Bank.YES:
            ...
            transaction_method = self.menu_get_available_transactions(
                transaction_metadata
            )  # pick desired method from menu
    ...
    def run(self):
        while True:
            try:
                ...
                input(
                    "Logon failed. Invalid credentials.  Please hit return to continue"
                )
            except Bank.CustomerAlreadyExistsException:
                input(
                    "Login Failed. Customer already exists.  Please hit return to continue"
                )
